...
/**
 * @file core.Modules.subscription/InstitutionalSubscriptionDAO.inc.php
 *
...
class InstitutionalSubscriptionDAO extends SubscriptionDAO {
	...
	public function getSubscriptionsByInstitutionName($institutionName, $journalId, $exactMatch = true, $rangeInfo = null) {
		// [WIZDAM FIX] Perbaikan logika konkatenasi string untuk PHP 7+
		$sql = 'SELECT s.*, iss.*
				FROM
				subscriptions s,
				subscription_types st,
	...
	public function subscriptionExistsByInstitutionName($institutionName, $journalId, $exactMatch = true) {
		// [WIZDAM FIX] Perbaikan logika konkatenasi string
		$sql = 'SELECT COUNT(*)
				FROM
				subscriptions s,
				subscription_types st,
	...
	public function _returnSubscriptionFromRow($row) {
		...
		HookRegistry::dispatch('InstitutionalSubscriptionDAO::_returnSubscriptionFromRow', array(&$institutionalSubscription, &$row));
	...
	public function _insertSubscriptionIPRanges($subscriptionId, $ipRanges) {
		...
		// [WIZDAM FIX] each() deprecated in PHP 7.2, removed in PHP 8.0. Replaced with foreach.
