...
class SectionDAO extends DAO {
    ...
    protected function _getCache() {
        if (!isset($this->cache)) {
            ...
            // Modern callback array without reference to $this
     ...
     */
    public function SectionDAO() {
        trigger_error(
            "Class '" . get_class($this) . "' uses deprecated constructor parent::SectionDAO(). Please refactor to use parent::__construct().",
            E_USER_DEPRECATED
        );
        self::__construct();
    }
     ...
     * @return Section
     */
    public function getSection($sectionId, $journalId = null, $useCache = false) {
        if ($useCache) {
     ...
     * @return Section
     */
    public function getSectionByAbbrev($sectionAbbrev, $journalId, $locale = null) {
     ...
     * @return Section
     */
    public function getSectionByTitle($sectionTitle, $journalId, $locale = null) {
        $sql = 'SELECT s.* FROM sections s, section_settings l WHERE l.section_id = s.section_id AND l.setting_name = ? AND l.setting_value = ? AND s.journal_id = ?';
     ...
     * @return Section
     */
    public function getSectionByTitleAndAbbrev($sectionTitle, $sectionAbbrev, $journalId, $locale) {
     ...
     * @return Section
     */
    public function _returnSectionFromRow($row) {
        $section = new Section();
        ...
        HookRegistry::dispatch('SectionDAO::_returnSectionFromRow', array($section, $row));
     ...
     * @param $journalId int optional
     */
    public function deleteSectionById($sectionId, $journalId = null) {
        ...
        $this->update('DELETE FROM section_settings WHERE section_id = ?', array((int)$sectionId));
        return $this->update('DELETE FROM sections WHERE section_id = ?', array((int)$sectionId));
    }
     ...
     * @param $journalId int
     */
    public function deleteSectionsByJournal($journalId) {
     ...
     * @return array
     */
    public function getSectionsForIssue($issueId) {
        $returner = array();
     ...
     */
    public function getJournalSections($journalId, $rangeInfo = null) {
     ...
     */
    public function getSections($rangeInfo = null) {
        $result = $this->retrieveRange(
     ...
     * @return array
     */
    public function getJournalEmptySectionIds($journalId) {
        $returner = array();
     ...
     * @return array
     */
    public function getSectionTitles($journalId, $submittableOnly = false) {
        $sections = array();
     ...
     * @return int
     */
    public function getInsertSectionId() {
    ...
    public function deleteCustomSection($issueId, $sectionId) {
        ...
        $this->update(
            'DELETE FROM custom_section_orders WHERE issue_id = ? AND section_id = ?', array((int)$issueId, (int)$sectionId)
        );

        // Reduce the section order of every successive section by one
        $this->update(
            'UPDATE custom_section_orders SET seq = seq - 1 WHERE issue_id = ? AND seq > ?', array((int)$issueId, (int)$sequence)
        );
    }
    ...
    public function resequenceCustomSectionOrders($issueId) {
        ...
        for ($i=1; !$result->EOF; $i++) {
            ...
            $this->update(
                ...
                array($i, (int)$sectionId, (int)$issueId)
            );
     ...
     */
    public function moveCustomSectionOrder($issueId, $sectionId, $newPos, $up) {
        $this->update(
            'UPDATE custom_section_orders SET seq = ? ' . ($up?'-':'+') . ' 0.5 WHERE issue_id = ? AND section_id = ?',
            array($newPos, (int)$issueId, (int)$sectionId)
        );
