...
class LayoutEditorSubmissionDAO extends DAO {
     ...
     */
    public function LayoutEditorSubmissionDAO() {
     ...
     * @return LayoutEditorSubmission|null
     */
    public function getSubmission($articleId, $journalId = null) {
        ...
        if ($result->RecordCount() != 0) {
            ...
            $returner = $this->_returnSubmissionFromRow($row);
        }
     ...
     */
    public function _returnSubmissionFromRow($row) {
        ...
        HookRegistry::dispatch('LayoutEditorSubmissionDAO::_returnLayoutEditorSubmissionFromRow', [&$submission, &$row]);
     ...
     * @param int $editorId
     * @param int|null $journalId
     * @param int|null $searchField SUBMISSION_FIELD_... constant
     * @param string|null $searchMatch 'is' or 'contains' or 'startsWith'
     * @param string|null $search Search string
     * @param int|null $dateField SUBMISSION_FIELD_DATE_... constant
     * @param int|null $dateFrom Search from timestamp
     * @param int|null $dateTo Search to timestamp
     * @param boolean $active true to select active assignments, false to select completed assignments
     * @param object|null $rangeInfo
     * @param string|null $sortBy
     * @param int $sortDirection
     * @return object DAOResultFactory
     */
    public function getSubmissions($editorId, $journalId = null, $searchField = null, $searchMatch = null, $search = null, $dateField = null, $dateFrom = null, $dateTo = null, $active = true, $rangeInfo = null, $sortBy = null, $sortDirection = SORT_DIRECTION_ASC) {
        $primaryLocale = AppLocale::getPrimaryLocale();
        $locale = AppLocale::getLocale();
        $params = [
            'title', // Section title
    ...
    public function _generateUserNameSearchSQL($search, $searchMatch, $prefix, &$params) {
        ...
        // [WIZDAM] Explicit push for clarity and PHP 8 compatibility
        $params[] = $search;
        $params[] = $search;
        $params[] = $search;
        $params[] = $search;
     ...
     * @param int $layoutEditorId
     * @param int $journalId
     * @return array
     */
    public function getSubmissionsCount($layoutEditorId, $journalId) {
        $sectionEditorSubmissionDao = DAORegistry::getDAO('SectionEditorSubmissionDAO');
        $stats = $sectionEditorSubmissionDao->getLayoutEditorStatistics($journalId, $layoutEditorId);
        return [
            0 => $stats[$layoutEditorId]['incomplete'] ?? 0,
            1 => $stats[$layoutEditorId]['complete'] ?? 0
        ];
    }

    /**
     * Get proofed articles by issue ID
     * @param int $issueId
     * @return array
     */
    public function getProofedArticlesByIssueId($issueId) {
        $articleIds = [];
        ...
        $result = $this->retrieve(
            ...
            [ASSOC_TYPE_ARTICLE, (int) $issueId, 'SIGNOFF_LAYOUT']
        );
