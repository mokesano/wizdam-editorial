...
/**
 * @file core.Modules.submission/sectionEditor/SectionEditorSubmission.inc.php
 *
...
class SectionEditorSubmission extends Article {
    ...
    public $removedReviewAssignments = [];
    ...
    public function addReviewAssignment($reviewAssignment) {
        ...
        $roundReviewAssignments[] = $reviewAssignment;
    ...
    public function updateReviewAssignment($reviewAssignment) {
        ...
        for ($i=0, $count=count($roundReviewAssignments); $i < $count; $i++) {
            if ($roundReviewAssignments[$i]->getReviewId() == $reviewAssignment->getId()) {
                ...
                $reviewAssignments[] = $roundReviewAssignments[$i];
            }
        }
     ...
     * @param object $editorFile ArticleFile
     */
        ...
        if ($inEditing) {
            ...
            if (is_array($reviewAssignments) && !empty($reviewAssignments)) {
                ...
                if (    ($lastDecisionDate || $commentDate) &&
                    $authorFileDate &&
                    $authorFileDate > max((int)$lastDecisionDate, $commentDate)
