...
<xsl:stylesheet
    ...
    exclude-result-prefixes="oai oai_dc dc oai_marc marcxml rfc1807 nlm xlink"
>
...
<xsl:template match="tk:toolkit" xmlns:tk="http://oai.dlib.vt.edu/OAI/metadata/toolkit">
  <h2>Platform Informations</h2>
  <table class="values">
    <tr><td class="key">Software Informations</td>
    <td class="value"><xsl:value-of select="tk:title"/></td></tr>
    <tr><td class="key">Author Name</td>
    <td class="value"><xsl:value-of select="tk:author/tk:name"/></td></tr>
    <tr><td class="key">Author Email</td>
    <td class="value"><xsl:value-of select="tk:author/tk:email"/></td></tr>
    <tr><td class="key">Version</td>
    <td class="value"><xsl:value-of select="tk:version"/></td></tr>
    <tr><td class="key">URL</td>
    <td class="value"><a href="{tk:URL}"><xsl:value-of select="tk:URL"/></a></td></tr>
  </table>
</xsl:template>
...
<xsl:template match="*[local-name()='oai_marc']">
    <div class="marcData">
        <h3>MARC Metadata (oai_marc)</h3>
        <table class="values">
            
            <xsl:for-each select="*[local-name()='varfield'][@id='245']">
                <tr>
                    <td class="key">Title</td>
                    <td class="value">
                        <xsl:value-of select="*[local-name()='subfield'][@label='a']"/>
                    </td>
                </tr>
            </xsl:for-each>

            <xsl:for-each select="*[local-name()='varfield'][@id='100' or @id='720']">
                <tr>
                    <td class="key">Author/Creator</td>
                    <td class="value">
                        <xsl:value-of select="*[local-name()='subfield'][@label='a']"/>
                    </td>
                </tr>
                <xsl:if test="*[local-name()='subfield'][@label='u']">
                    <tr>
                        <td class="key">Affiliation</td>
                        <td class="value">
                            <xsl:value-of select="*[local-name()='subfield'][@label='u']"/>
                        </td>
                    </tr>
                </xsl:if>
                <xsl:if test="*[local-name()='subfield'][@label='0']">
                    <tr>
                        <td class="key">Profile/ORCID</td>
                        <td class="value">
                            <a href="{*[local-name()='subfield'][@label='0']}">
                                <xsl:value-of select="*[local-name()='subfield'][@label='0']"/>
                            </a>
                        </td>
                    </tr>
                </xsl:if>
            ...
            <xsl:for-each select="*[local-name()='varfield'][@id='653']">
                <tr>
                    <td class="key">Subject</td>
                    <td class="value"><xsl:value-of select="*[local-name()='subfield'][@label='a']"/></td>
                </tr>
            </xsl:for-each>

            <xsl:for-each select="*[local-name()='varfield'][@id='520']">
                <tr>
            ...
            <xsl:for-each select="*[local-name()='varfield'][@id='260']">
                <xsl:if test="*[local-name()='subfield'][@label='b']">
                    <tr>
                        <td class="key">Publisher</td>
                        <td class="value"><xsl:value-of select="*[local-name()='subfield'][@label='b']"/></td>
                    </tr>
                </xsl:if>
            </xsl:for-each>

            <xsl:for-each select="*[local-name()='varfield'][@id='260']">
                <xsl:if test="*[local-name()='subfield'][@label='c']">
                    <tr>
                        <td class="key">Date</td>
                        <td class="value"><xsl:value-of select="*[local-name()='subfield'][@label='c']"/></td>
                    </tr>
                </xsl:if>
            </xsl:for-each>

            <xsl:for-each select="*[local-name()='varfield'][@id='655']">
                <tr>
            ...
            <xsl:for-each select="*[local-name()='varfield'][@id='856']">
                <xsl:if test="*[local-name()='subfield'][@label='q']">
                    <tr>
                        <td class="key">Format</td>
                        <td class="value"><xsl:value-of select="*[local-name()='subfield'][@label='q']"/></td>
                    </tr>
                </xsl:if>
            </xsl:for-each>
            ...
            <xsl:for-each select="*[local-name()='varfield'][@id='856']">
                <xsl:if test="*[local-name()='subfield'][@label='u']">
                    <tr>
                        ...
                        <td class="value">
                            <a href="{*[local-name()='subfield'][@label='u']}">
                                <xsl:value-of select="*[local-name()='subfield'][@label='u']"/>
                            </a>
                        </td>
                    </tr>
            ...
            <xsl:for-each select="*[local-name()='varfield'][@id='786']">
                <tr>
                    <td class="key">Source</td>
                    <td class="value"><xsl:value-of select="*[local-name()='subfield'][@label='n']"/></td>
                </tr>
            </xsl:for-each>

            <xsl:for-each select="*[local-name()='varfield'][@id='546']">
                <tr>
            ...
            <xsl:for-each select="*[local-name()='varfield'][@id='540']">
                <tr>
...
<xsl:template match="*[local-name()='record' and namespace-uri()='http://www.loc.gov/MARC21/slim']">
    <div class="marcData">
        <h3>MARC21-XML Data</h3>
        <table class="values">
            
            <xsl:if test="*[local-name()='leader']">
                <tr>
                    <td class="key">Leader</td>
                    <td class="value"><xsl:value-of select="*[local-name()='leader']"/></td>
                </tr>
            </xsl:if>

            <xsl:for-each select="*[local-name()='controlfield']">
                <tr>
                    <td class="key">Tag <xsl:value-of select="@tag"/></td>
                    <td class="value"><xsl:value-of select="."/></td>
                </tr>
            </xsl:for-each>

            <xsl:for-each select="*[local-name()='datafield']">
                <xsl:variable name="tag" select="@tag"/>
                <xsl:variable name="ind1" select="@ind1"/>
                <xsl:variable name="ind2" select="@ind2"/>

                <xsl:for-each select="*[local-name()='subfield']">
                    <tr>
                        <td class="key">
                            Tag <xsl:value-of select="$tag"/>
                            <xsl:if test="$ind1 != ' ' or $ind2 != ' '"> (<xsl:value-of select="$ind1"/><xsl:value-of select="$ind2"/>)</xsl:if>
...
<xsl:template match="*[local-name()='rfc1807']">
    <div class="rfc1807Data">
        <h3>RFC 1807 Metadata</h3>
        <table class="values">
            <xsl:for-each select="*">
                <tr>
                    <td class="key">
                        <xsl:value-of select="translate(substring(local-name(), 1, 1), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ')"/>
                        <xsl:value-of select="substring(local-name(), 2)"/>
                    </td>
                    <td class="value">
                        <xsl:choose>
                            <xsl:when test="local-name()='copyright' and starts-with(., '[http')">
                                <xsl:variable name="cleanUrl" select="substring-before(substring-after(., '['), ']')"/>
                                <xsl:variable name="explanation" select="substring-after(., ']')"/>
                                
                                <a href="{$cleanUrl}" class="link" target="_blank">URL</a>
...
<xsl:template match="*[local-name()='article']">
    <div class="nlmData">
        <h3>NLM / JATS Metadata</h3>
        
        <xsl:variable name="front" select="*[local-name()='front']" />
        <xsl:variable name="jmeta" select="$front/*[local-name()='journal-meta']" />
        <xsl:variable name="ameta" select="$front/*[local-name()='article-meta']" />

        <table class="values">
            <xsl:for-each select="$jmeta/*[local-name()='journal-title']">
                <tr><td class="key">Journal Title</td><td class="value"><xsl:value-of select="."/></td></tr>
            </xsl:for-each>
            
            <xsl:for-each select="$jmeta/*[local-name()='issn']">
                <tr><td class="key">ISSN (<xsl:value-of select="@*[local-name()='pub-type']"/>)</td><td class="value"><xsl:value-of select="."/></td></tr>
            </xsl:for-each>

            <xsl:for-each select="$jmeta/*[local-name()='publisher']/*[local-name()='publisher-name']">
                <tr><td class="key">Publisher</td><td class="value"><xsl:value-of select="."/></td></tr>
            </xsl:for-each>

            <xsl:for-each select="$ameta/*[local-name()='article-id']">
                <tr><td class="key">Identifier (<xsl:value-of select="@*[local-name()='pub-id-type']"/>)</td><td class="value"><xsl:value-of select="."/></td></tr>
            </xsl:for-each>

            <xsl:for-each select="$ameta/*[local-name()='title-group']/*">
                <xsl:if test="local-name()='article-title' or local-name()='trans-title'">
                    <tr>
                        <td class="key">
                            Title <xsl:if test="@*[local-name()='lang']"> (<xsl:value-of select="@*[local-name()='lang']"/>)</xsl:if>
                        </td>
                        <td class="value"><xsl:value-of select="."/></td>
                    </tr>
                </xsl:if>
            ...
            <xsl:for-each select="$ameta/*[local-name()='contrib-group']/*[local-name()='contrib']">
                <tr>
                    <td class="key">
                        Contributor <xsl:if test="@*[local-name()='contrib-type']"> (<xsl:value-of select="@*[local-name()='contrib-type']"/>)</xsl:if>
                    </td>
                    <td class="value">
                        <xsl:if test="*[local-name()='name']">
                            <xsl:value-of select="*[local-name()='name']/*[local-name()='given-names']"/>
                            <xsl:text> </xsl:text>
                            <xsl:value-of select="*[local-name()='name']/*[local-name()='surname']"/>
                        </xsl:if>
                        <xsl:if test="*[local-name()='aff']">
                            <br/>Affiliation: <xsl:value-of select="*[local-name()='aff']"/>
                        </xsl:if>
                        <xsl:if test="*[local-name()='email']">
                            <br/>Email: <xsl:value-of select="*[local-name()='email']"/>
                        </xsl:if>
                    </td>
                </tr>
            </xsl:for-each>

            <xsl:for-each select="$ameta/*[local-name()='pub-date']">
                <tr>
                    <td class="key">Date</td>
                    <td class="value">
                        <xsl:value-of select="*[local-name()='year']"/>-<xsl:value-of select="*[local-name()='month']"/>-<xsl:value-of select="*[local-name()='day']"/>
                    </td>
                </tr>
            </xsl:for-each>

            <xsl:for-each select="$ameta/*">
                <xsl:if test="local-name()='abstract' or local-name()='abstract-trans'">
                    <tr>
                        <td class="key">
                            Abstract <xsl:if test="@*[local-name()='lang']"> (<xsl:value-of select="@*[local-name()='lang']"/>)</xsl:if>
                        </td>
                        <td class="value"><xsl:value-of select="*[local-name()='p']"/></td>
                    </tr>
                </xsl:if>
            </xsl:for-each>

            <xsl:for-each select="$ameta/*[local-name()='kwd-group']">
                <tr>
                    <td class="key">
                        Keywords <xsl:if test="@*[local-name()='lang']"> (<xsl:value-of select="@*[local-name()='lang']"/>)</xsl:if>
                    </td>
                    <td class="value">
            ...
            <xsl:if test="$ameta/*[local-name()='permissions']/*[local-name()='copyright-statement']">
                <tr>
                    <td class="key">Rights</td>
                    <td class="value"><xsl:value-of select="$ameta/*[local-name()='permissions']/*[local-name()='copyright-statement']"/></td>
                </tr>
            </xsl:if>

            <xsl:for-each select="$ameta/*[local-name()='self-uri']">
                <tr>
                    <td class="key">Resource Link</td>
                    <td class="value">
                        <a href="{@*[local-name()='href']}"><xsl:value-of select="@*[local-name()='href']"/></a>
                    </td>
                </tr>
            </xsl:for-each>
