...
(function($) {
	$.fn.splitter = function(args) {
		...
		return this.each(function() {
			...
			function coercePane(pane, paneName) {
				if (pane && pane.jquery) {
					return pane;
				}
				if (pane && pane.nodeType) {
					return $(pane);
				}
				if (typeof pane === 'string') {
					var nodes = $.find(pane, splitter[0]);
					if (!nodes || !nodes.length) {
						throw new Error("splitter: option '" + paneName + "' must reference an existing element.");
					}
					return $(nodes[0]);
				}
				throw new Error("splitter: option '" + paneName + "' must be a jQuery object, DOM element, or selector.");
			}

			var A = coercePane(args.A, 'A'); // left/top frame
			var B = coercePane(args.B, 'B'); // right/bottom frame
