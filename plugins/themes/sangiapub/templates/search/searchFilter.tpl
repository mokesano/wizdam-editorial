...
<fieldset class="c-facet__item u-pa-0" data-test="field-set-article-{translate key=$key}">
    <legend>
        <span class="c-facet__label label"><label for="{$filterName}">{translate key=$key}</label></span>
        <span class="grade-c-hide u-js-show">
			...
			{else}
				...
				{if empty($filterInput)}
					<input type="text" name="{$filterName}" id="{$filterName}" size="25" maxlength="255" value="{$filterValue|escape}" class="textField" />
				{else}
					{$filterInput}
				{/if}
			{/if}
