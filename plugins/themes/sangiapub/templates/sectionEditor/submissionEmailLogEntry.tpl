...
<p class="u-mt-16">
{if $isEditor}
	<a href="{url page="editor" op="clearSubmissionEmailLog" path=$submission->getId()|to_array:$logEntry->getId()}" onclick="return confirm('{translate|escape:"jsparam" key="submission.email.confirmDeleteLogEntry"}')" class="link-button">{translate key="submission.email.deleteLogEntry"}</a>
{/if}
    <a href="{url op="submissionEmailLog" path=$submission->getId()}" class="link-button">{translate key="submission.email.backToEmailLog"}</a>
</p>
