...
<div id="selectReviewer">
    ...
    <div class="c-jump">
        ...
        <span>{foreach from=$alphaList item=letter}<a href="{url op="selectReviewer" path=$articleId searchInitial=$letter}" class="c-jump-navigation__link u-margin-bottom-xxs-at-md u-mr-4">{if $letter == $searchInitial}<strong>{$letter|escape}</strong>{else}{$letter|escape}{/if}</a>{/foreach}<a href="{url op="selectReviewer" path=$articleId}" class="c-jump-navigation__link u-margin-bottom-xxs-at-md u-mr-4">{if $searchInitial==''}<strong>{translate key="common.all"}</strong>{else}{translate key="common.all"}{/if}</a></span>
    </div>
    
    <p class="u-mb-16"><a class="action" href="{url op="enrollSearch" path=$articleId}">{translate key="sectionEditor.review.enrollReviewer"}</a>&nbsp;|&nbsp;<a class="action" href="{url op="createReviewer" path=$articleId}">{translate key="sectionEditor.review.createReviewer"}</a>{foreach from=$reviewerDatabaseLinks item="link"}{if !empty($link.title) && !empty($link.url)}&nbsp;|&nbsp;<a href="{$link.url|escape}" target="_new" class="action">{$link.title|escape}</a>{/if}{/foreach}</p>
    ...
    <table class="listing" width="100%">
        ...
        {iterate from=reviewers item=reviewer}
            ...
            <tr valign="top"{if $reviewer->review_id && !$reviewer->declined} class="{translate key="common.alreadyAssigned"}"{/if}>
        ...
        {if $reviewers->wasEmpty()}
            ...
            <tr>
                <td colspan="{$numCols|escape}" class="endseparator">&nbsp;</td>
            </tr>
        {else}
        	<tr class="u-hide">
    ...
    <div class="colspan u-mb-0" id="colspan">	    
    	<section class="u-display-flex u-justify-content-center u-mt-24 u-mb-24">
    	    <div class="c-pagination">{page_info iterator=$reviewers}</div>
        </section>
        {if $reviewers->getPageCount() > 1}
        <section class="u-display-flex u-justify-content-center">
            <div class="c-pagination">{page_links anchor="reviewers" name="reviewers" iterator=$reviewers searchInitial=$searchInitial searchField=$searchField searchMatch=$searchMatch search=$search dateFromDay=$dateFromDay dateFromYear=$dateFromYear dateFromMonth=$dateFromMonth dateToDay=$dateToDay dateToYear=$dateToYear dateToMonth=$dateToMonth sort=$sort sortDirection=$sortDirection}
           </div>
        </section>
        {/if}
    </div>
    ...
    <h4 class="u-h3">{translate key="common.notes"}</h4>
