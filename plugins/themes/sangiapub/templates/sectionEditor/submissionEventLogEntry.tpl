...
<div id="submissionEventLog">
    ...
    {if $isEditor}
    	<p><a href="{url page="editor" op="clearSubmissionEventLog" path=$submission->getId()|to_array:$logEntry->getId()}" onclick="return confirm('{translate|escape:"jsparam" key="submission.event.confirmDeleteLogEntry"}')" class="link-button">{translate key="submission.event.deleteLogEntry"}</a>
    	</p>
    {/if}
</div>

<p><a class="link-button" href="{url op="submissionEventLog" path=$submission->getId()}">{translate key="submission.event.backToEventLog"}</a>
</p>
