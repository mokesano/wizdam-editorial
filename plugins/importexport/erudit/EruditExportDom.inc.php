...
class EruditExportDom {
    ...
    public static function generateArticleDom($doc, $journal, $issue, $article, $galley): DOMElement {
        ...
        if ($issue->getDatePublished()) {
            XMLCustomWriter::createChildWithText($doc, $digPubNode, 'date', self::formatDate($issue->getDatePublished()));
        }
        ...
        if ($keywords = $article->getSubject($article->getLocale())) {
            ...
            XMLCustomWriter::setAttribute($keywordGroupNode, 'lang', $language ? $language : 'en');
