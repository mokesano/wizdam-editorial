...
class Depositor extends ScheduledTask {
    ...
    public function Depositor($args) {
        if (Config::getVar('debug', 'deprecation_warnings')) {
            trigger_error("Class '" . get_class($this) . "' uses deprecated constructor parent::Depositor(). Please refactor to parent::__construct().", E_USER_DEPRECATED);
        }
        $args = func_get_args();
    ...
    protected function _processNewDepositObjects($journal) {
        ...
        // create new deposit objects for any new App content
        $depositDao = DAORegistry::getDAO('DepositDAO');
        $depositObjectDao = DAORegistry::getDAO('DepositObjectDAO');
