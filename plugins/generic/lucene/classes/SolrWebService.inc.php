...
class SolrWebService extends XmlWebService {
	...
	/**
	 * DEPRECATED: Backward compatibility constructor (PHP 4 style)
	 * @deprecated Use __construct() instead
	 */
	public function SolrWebService($searchHandler, $username, $password, $instId, $useProxy = false) {
		if (Config::getVar('debug', 'deprecation_warnings')) {
			trigger_error(
				"DEPRECATED: Class '" . get_class($this) . "' uses deprecated constructor parent::SolrWebService(). " .
				"ACTION: Refactor to use parent::__construct() instead of SolrWebService().",
				E_USER_DEPRECATED
			);
		}
        $args = func_get_args();
	 ...
	 * large as possible to reduce index commit overhead.
	 *
	 * @param $pullIndexingCallback callback This function will be called
	 ...
	 */
	private function _makeRequest($url, $params = array(), $method = 'GET') {
	 ...
	 *
	 * @param $articles array The articles to be included in the list.
	 ...
	 * @return string The translated search phrase.
	 */
	...
	private function _getSuggesterAutosuggestions($url, $userInput, $fieldName) {
		...
		// Make the request.
		$response = $this->_makeRequest($url, $params);
		if (!($response instanceof DOMXPath)) return array();
	...
	private function _isArticleAccessAuthorized($article) {
		...
		if (!($article instanceof PublishedArticle)) return false;
