...
class BookForReviewForm extends Form {
    ...
    /** @var BookForReview the book being edited */
    public $book;

    /** @var array keys are valid status values */
    public $validStatus;

    /** @var array keys are valid author type values */
    public $validAuthorTypes;

    /** @var array keys are valid language code values */
    public $validLanguages;

    /** @var array keys are valid edition values */
    public $validEditions;
    ...
    public function __construct($parentPluginName, $bookId = null) {
        ...
        // [MODERNISASI] Ganti each() dengan foreach
        ...
        // [MODERNISASI] Ganti create_function dengan Closure
     ...
     */
    public function BookForReviewForm($parentPluginName, $bookId = null) {
        if (Config::getVar('debug', 'deprecation_warnings')) {
            trigger_error(
                "Class '" . get_class($this) . "' uses deprecated constructor parent::BookForReviewForm(). Please refactor to parent::__construct().", 
                E_USER_DEPRECATED
            );
        }
        self::__construct($parentPluginName, $bookId);
    }
    ...
    public function readInputData() {
        ...
        if (!empty($this->_data['url'])) {
            ...
            $this->addCheck(new FormValidatorCustom($this, 'url', 'required', 'plugins.generic.booksForReview.editor.form.urlPrefixIncluded', function($url) {
                // Pastikan fungsi strtolower_codesafe tersedia atau gunakan strtolower
    ...
    public function execute($object = null) {
        ...
        // [FIX] Validasi authors array sebelum loop
        if (is_array($authors)) {
            for ($i=0, $count=count($authors); $i < $count; $i++) {
        ...
        $deletedAuthorsVar = $this->getData('deletedAuthors');
        if ($deletedAuthorsVar) {
            $deletedAuthors = explode(':', $deletedAuthorsVar);
        ...
        if ($publicFileManager->uploadedFileExists(BFR_COVER_PAGE_IMAGE_NAME)) {
            ...
            $type = $publicFileManager->getUploadedFileType(BFR_COVER_PAGE_IMAGE_NAME);
            $extension = $publicFileManager->getImageExtension($type);
            $newFileName = 'cover_bfr_' . $book->getId() . '_' . $formLocale . $extension; // fixed missing extension var
