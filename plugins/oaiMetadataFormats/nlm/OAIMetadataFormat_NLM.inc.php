...
class OAIMetadataFormat_NLM extends OAIMetadataFormat {
    ...
    public function toXml($record, $format = null) {
        ...
        // Use date() instead of strftime() (deprecated in PHP 8.1)
        ...
        $response .=
            "\t\t\t<permissions>\n" .
            "\t\t\t\t<copyright-statement>" . htmlspecialchars(__('submission.copyrightStatement', ['copyrightYear' => $article->getCopyrightYear(), 'copyrightHolder' => $article->getLocalizedCopyrightHolder()])) . "</copyright-statement>\n" .
        ...
        foreach ($article->getGalleys() as $galley) {
            $response .= "\t\t\t<self-uri content-type=\"" . htmlspecialchars(Core::cleanVar($galley->getFileType())) . "\" xlink:href=\"" . htmlspecialchars(Core::cleanVar(Request::url($journal->getPath(), 'article', 'view', [$article->getBestArticleId(), $galley->getId()]))) . "\" />\n";
        }
    ...
    /**
     * Retrieve and format editorial information for a journal.
     * @param int $journalId
     * @return string
     */
    public function getEditorialInfo($journalId) {
        static $editorialInfo = [];
        if (isset($editorialInfo[$journalId])) return $editorialInfo[$journalId];

        $response = '';
        $roleDao = DAORegistry::getDAO('RoleDAO'); /* @var $roleDao RoleDAO */
        $roleMap = [ROLE_ID_EDITOR => 'editor', ROLE_ID_SECTION_EDITOR => 'secteditor', ROLE_ID_JOURNAL_MANAGER => 'jmanager'];
